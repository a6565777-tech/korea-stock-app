...
[예약매수 기본 규칙]
- 체결 조건: 시가 ±0.5% 이내일 때만 체결 (갭 크면 자동 스킵)
- 목표가: 시가 +2.0% (9:00~11:00 중 도달 시 매도)
- 손절가: 시가 -2.0% (즉시 매도, 절대 기다리지 말 것)

[상승 확률 - 0~100% 정수]
"장 시작 후 11시까지 시가 대비 +2% 도달 확률"을 숫자로.
...
• 삼성전자 [신호] | 상승확률 [N]%
  └ 한 줄 근거 (미국장 반응 + 수급 + 뉴스)
  📊 전일 종가 ₩[X,XXX] / 갭 예상 [+N% ~ +N% 또는 -N%]
  💰 예약매수: 시가 ±0.5% / 🎯 시가 +2.0% / 🛑 시가 -2.0%
  ⏱️ 데이 (9:00~11:00)
• SK하이닉스 [신호] | 상승확률 [N]%
  └ 한 줄 근거
  📊 전일 종가 ₩[X,XXX] / 갭 예상 [±N%]
...
💡 갭업 크면 과감히 스킵. 손절가 -2%는 절대 지킬 것. 시초가 체크 필수.
"""
...
- 휴림로봇 같은 테마주는 강한 호재 + 수급 확인 없으면 기본 ⚪관망.
