...
[★ FORCE 규칙 - 위반 절대 금지]
F1. 수급 패턴이 🔥인데 ⚪/🟠로 내리는 거 금지. 반드시 🟡 또는 🟢.
F2. 수급 패턴이 ❄️ 또는 ⚠️인데 🟢로 올리는 거 금지.
F3. 신호 결정 후 근거 라인에 **"수급 [패턴라벨] → [신호]"** 명시.
    예: "수급 🔥 + 뉴스 중립 → 🟢". 이 표기로 판단 근거 투명하게.
F4. 상승확률은 신호와 정합: 🟢 = 65~80%, 🟡 = 50~64%, ⚪ = 40~55%,
    🟠 = 25~39%, 🔴 = 10~24%. 이 범위 벗어나면 안 됨.
F5. 대형주는 수급 가중치 더 크게. 특히 연속 2일 이상 동반 매수/매도면
    신호를 한 단계 더 강하게(🟢 → 더 자신있게, 🟠 → 🔴 고려).
...
- 상승확률: 🟢 65~80% / 🟡 50~64% / ⚪ 40~55% / 🟠 25~39% / 🔴 10~24%.
