...
from src.analyzers import scoring as _scoring
...
def _log(msg: str) -> None:
    ts = timez.now().strftime("%H:%M:%S")
    line = f"[{ts}] {msg}"
    print(line)
    # 읽기전용 FS(Vercel 등)에서는 파일 로그 스킵
    try:
        _LOG_DIR.mkdir(exist_ok=True)
        logfile = _LOG_DIR / f"briefing_{timez.now().strftime('%Y-%m-%d')}.log"
...
[신호 체계 - 반드시 이 중 하나]
🟢 매수 추천    : ★빡빡함★ 다음 4조건 중 3개 이상 동시 충족시에만:
                  (a) 외인+기관 동반 순매수 2일 이상 지속
                  (b) 명시적 호재 (실적·계약·정책 등 구체 뉴스)
                  (c) 갭 예상 -0.5% ~ +1.5% 범위 (과열 갭 X)
                  (d) 일변동폭 1.5~4% (저변동 → 도달 어려움 / 고변동 → 손절 위험)
                  3개 미만이면 자동 🟡로 다운그레이드.
🟡 매수 조심    : 부분 시그널. 매수 의향은 있으나 확신 부족.
                  (실제 적중률이 🟢보다 자주 높음 — 자신감 캘리브레이션 부족이라
                   판단되면 🟢 후보를 🟡로 자진 다운그레이드해도 좋음.)
⚪ 관망         : 방향성 불명확 또는 신호 약함. 예약매수 보류.
🟠 매수 비추천  : ★발급 신중★ 명확한 약세 근거(수급 이탈 + 갭다운 -1.5%↓ + 악재) 전부 충족 시.
                  단순 "수급 엇갈림" "뉴스 없음"은 ⚪ 관망으로 분류 (🟠 남발 금지).
...
[★ 원화 환산 병기 - 필수 (🟢/🟡 매수 추천/조심 신호에만 적용)]
% 수치만 쓰지 마. 반드시 실제 원화 가격을 같이 써서 사용자가 바로 주문 낼 수 있게.

1) 먼저 예상 시가 계산:
   예상시가 = 전일 종가 × (1 + 예상 갭%)
   예: 전일 종가 ₩1,155,000 + 갭 +1.0% → 예상 시가 ≈ ₩1,166,550

2) 그 예상시가 기준으로 모든 가격 산출:
   체결하한 = 예상시가 × (1 - 체결폭/100)
   체결상한 = 예상시가 × (1 + 체결폭/100)
   목표가   = 예상시가 × (1 + Y/100)
   손절가   = 예상시가 × (1 - Z/100)
...
4) 반올림 규칙 (보기 깔끔하게):
   - 10만원 미만   → 100원 단위 반올림
   - 10만원~100만원 → 500원 단위 반올림
   - 100만원 이상   → 1,000원 단위 반올림

5) ⚪관망·🟠비추천·🔴매도 신호는 체결/목표 원화 생략, 손절 원화만 유지.

[★ 상승 확률 - 0~100% 정수 — 현실적 범위 강제]
"장 시작 후 11시까지 목표가 도달 확률"을 숫자로.

⚠️ 중요: 한국 대형주 2시간 내 ±1.5% 목표 도달 실제 빈도 = 25~35%. 즉,
"70%+ 확률"은 **구조적으로 존재하지 않는다**. 프로 트레이더 승률도 55% 수준.
Gemini가 자신 있어 보이려고 높은 숫자 찍으면 그냥 틀린 답이다.

[강제 범위 — 이 안에서만 고를 것]
  🟢 매수 추천    : 52~58%  (역사적 상한. 강한 호재+수급이어도 이게 MAX)
  🟡 매수 조심    : 48~54%
  ⚪ 관망         : 45~52%  (동전던지기 영역)
  🟠 매수 비추천  : 38~48%
  🔴 매도 고려    : 28~42%
...
[금지]
- 60% 초과 절대 금지. "확신"있어 보여도 구조적 불가.
- 25% 미만 절대 금지. 반등 가능성은 언제나 있다.
- 극단값(예: 72%, 18%) 찍으면 시스템이 걸러냄. 현실적 범위 내에서만.
- 컨텍스트에 "역사적 도달률: X%"가 있으면 ±3%만 조정 가능. 지어내기 금지.

[★★★ 수급 분석 - 이 브리핑의 최우선 판단 기준 ★★★]
뉴스는 이미 시장에 반영됐을 수 있지만 외인·기관·개인 수급은 "지금 이 순간 누가 들어오고
누가 나가는지"를 보여주는 가장 직접적인 선행지표.

**신호는 수급 패턴으로 먼저 결정하고, 뉴스로 ±1단계 조정한다.** 뉴스가 아무리 좋아도
수급이 나쁘면 🟢 금지. 수급이 좋으면 뉴스 안 좋아도 🟠 금지.

[수급 → 최종 신호 결정 테이블 - 반드시 이대로 매핑할 것]
┌──────────────────────────────┬──────────────────┬────────────────────┐
│  수급 패턴 (컨텍스트 라벨)   │  뉴스 상태       │  최종 신호          │
├──────────────────────────────┼──────────────────┼────────────────────┤
│ 🔥 외인+기관 동반 순매수     │ 호재 or 중립     │ 🟢 매수 추천         │
│ 🔥 외인+기관 동반 순매수     │ 약한 악재        │ 🟡 매수 조심         │
│ 🔥 외인+기관 동반 순매수     │ 강한 악재        │ ⚪ 관망 (🟠 금지)    │
│ ❄️ 외인+기관 동반 순매도     │ 호재             │ ⚪ 관망 (🟢 절대 X)  │
│ ❄️ 외인+기관 동반 순매도     │ 중립 or 악재     │ 🟠 매수 비추천       │
│ ❄️ 외인+기관 동반 순매도     │ 강한 악재+추세   │ 🔴 매도 고려         │
│ ⚠️ 개미 물림 (외기관 매도+개인 매수) │ 무관     │ 🟠 매수 비추천 (절대 🟢 X) │
│ ⚠️ 개미 물림 + 강한 악재     │ —                │ 🔴 매도 고려         │
│ 🟡 외인·기관 엇갈림           │ 뚜렷한 호재      │ 🟡 매수 조심         │
│ 🟡 외인·기관 엇갈림           │ 중립             │ ⚪ 관망              │
│ 🟡 외인·기관 엇갈림           │ 악재             │ 🟠 매수 비추천       │
│ ❓ 수급 데이터 없음           │ 무관             │ 한 단계 보수적       │
└──────────────────────────────┴──────────────────┴────────────────────┘

[★ FORCE 규칙 - 위반 절대 금지]
F1. 🔥 패턴 → 반드시 🟡 또는 🟢. ⚪/🟠/🔴 금지.
F2. ❄️ 패턴 → 반드시 ⚪/🟠/🔴 중 하나. 🟢/🟡 금지.
F3. ⚠️ 개미 물림 패턴 → 반드시 🟠 또는 🔴. 🟢/🟡/⚪ **전부 금지**.
    (개미 물림은 하락 확정적. 매수 관망도 안 됨, 진입 자체 X.)
F4. 근거 라인에 **"수급 [패턴라벨] → [신호]"** 명시.
    예: "수급 🔥 + 뉴스 중립 → 🟢", "수급 ⚠️ + 뉴스 무관 → 🟠".
F5. 상승확률은 신호와 정합, 현실적 범위 엄수:
    🟢 = 52~58%, 🟡 = 48~54%, ⚪ = 45~52%, 🟠 = 38~48%, 🔴 = 28~42%.
    이 범위 벗어나면 안 됨. 60% 초과·25% 미만 절대 금지.
F6. 대형주는 수급 가중치 더 크게. 연속 2일 이상 동반 매수/매도면
    신호를 한 단계 더 강하게.
...
- **원화 환산 계산 정확성 필수**. 검산 후 출력. 반올림 규칙 엄수.
- **수급은 모든 종목에 구체 수치로 1줄 필수** — "외인 168만주 / 기관 143만주 매수, 개인 -496만주"
  형식. 그냥 "수급 양호" 같은 모호한 표현 금지.
- 각 종목 블록 사이에 빈 줄 하나 넣어서 가독성 확보.
- 한국어, 간결하게.

[★ 참고 예시 - 이 톤과 신호 결정 방식을 그대로 따를 것]

예시 A) 수급 🔥 + 뉴스 중립 → 🟢 매수 추천
• 삼성전자 🟢 매수 추천 | 상승확률 72%
  └ 수급 🔥 + 뉴스 중립 → 🟢. HBM 수요 견고, 외기관 동반 매수로 수급 강세.
  💹 수급: 외인 +168.9만주 / 기관 +143.3만주 / 개인 -496.8만주 → 🔥 동반 순매수
  📊 전일 종가 ₩217,500 / 갭 예상 +0.5% / 일변동 2.8%
  📈 예상 시가 ≈ ₩218,500
  💰 체결: 시가 ±0.5% ≈ ₩217,400~₩219,600
  🎯 목표: 시가 +1.5% ≈ ₩221,800 (1주당 +₩3,300)
  🛑 손절: 시가 -1.0% ≈ ₩216,300 (1주당 -₩2,200)
  ⏱️ 데이 (9:00~11:00)

예시 B) 수급 ⚠️ 개미 물림 → 🟠 매수 비추천
• 휴림로봇 🟠 매수 비추천 | 상승확률 28%
  └ 수급 ⚠️ + 뉴스 무관 → 🟠. 외인 67만주 대량 매도 + 개인 혼자 매수, 전형적 개미 물림.
  💹 수급: 외인 -67.1만주 / 기관 +0.3만주 / 개인 +66.3만주 → ⚠️ 개미 물림
  📊 전일 종가 ₩11,540 / 갭 예상 -0.5% / 일변동 6.3%
  🛑 손절: 시가 -4.0% ≈ ₩11,000 (1주당 -₩460)
  ⏱️ 데이 (진입 금지)

예시 C) 수급 ❄️ + 호재 뉴스 → ⚪ 관망 (🟢 금지!)
• OO전자 ⚪ | 상승확률 48%
  └ 수급 ❄️ + 호재 → ⚪. 호재 있으나 외기관 동반 매도 확인, 추격 매수 보류.
  💹 수급: 외인 -25.3만주 / 기관 -18.1만주 / 개인 +42.5만주 → ❄️ 동반 순매도
  📊 전일 종가 ₩85,000 / 갭 예상 +1.0% / 일변동 2.5%
  🛑 손절: 시가 -1.2% ≈ ₩85,000 (1주당 -₩1,000)
  ⏱️ 데이 (수급 반전 확인 후 재고)
"""
...
⚠️ 단기(1~4주) 등락 예측은 구조적으로 노이즈가 크다. 공개 정보로 55% 넘기기 어려움.
Gemini가 자신 있어 보이려고 80%+ 찍으면 그냥 틀린 답이다.
...
[금지]
- 60% 초과 절대 금지. 근거 아무리 많아도 이게 천장.
- 25% 미만 금지.
- 컨텍스트에 "역사적 도달률: X%"가 있으면 ±3%만 조정.
...
[★★★ 수급 분석 - 최우선 판단 기준 ★★★]
외국인·기관·개인 수급은 단기 주가의 가장 강한 선행지표.
**신호는 수급으로 먼저 결정, 뉴스로 ±1단계 조정.**

[수급 → 최종 신호 결정 테이블]
  🔥 외인+기관 동반 매수 + 호재/중립  → 🟢 매수 추천
  🔥 외인+기관 동반 매수 + 악재        → 🟡 매수 조심
  ❄️ 외인+기관 동반 매도 + 호재        → ⚪ 관망 (🟢 금지)
  ❄️ 외인+기관 동반 매도 + 중립/악재   → 🟠 매수 비추천 (심하면 🔴)
  ⚠️ 개미 물림 (외기관 매도+개인 매수) → 🟠 매수 비추천 (🟢 금지)
  🟡 엇갈림 + 뚜렷한 호재               → 🟡 매수 조심
  🟡 엇갈림 + 중립                       → ⚪ 관망
  🟡 엇갈림 + 악재                       → 🟠 매수 비추천
  ❓ 수급 데이터 없음                    → 한 단계 보수적
...
- 🔥 패턴이면 ⚪/🟠 금지. 반드시 🟡 또는 🟢.
- ❄️ / ⚠️ 패턴이면 🟢 금지.
- 근거 라인에 **"수급 [패턴] → [신호]"** 표기 필수.
- 상승확률 범위 엄수: 🟢 54~60% / 🟡 48~55% / ⚪ 45~52% / 🟠 38~48% / 🔴 28~42%.
  60% 초과 금지. 25% 미만 금지. 지어내지 마.
- 대형주(삼성·SK하이닉스·현대차·LG전자)는 수급 가중치 더 크게.

[참고 예시 - 이 톤 그대로]
• 삼성전자 🟢 매수 추천 | 상승확률 70%
  └ 수급 🔥 + 뉴스 중립 → 🟢. 외기관 동반 매수, 일관된 자금 유입.
  💹 수급: 외인 +168.9만주 / 기관 +143.3만주 / 개인 -496.8만주 → 🔥 동반 순매수
  💰 진입 ₩217,000~₩219,000 / 🎯 ₩225,000 / 🛑 ₩213,000
  ⏱️ 단기

• 휴림로봇 🟠 매수 비추천 | 상승확률 30%
  └ 수급 ⚠️ + 뉴스 무관 → 🟠. 외인 대량 매도 + 개인 혼자 매수, 개미 물림.
  💹 수급: 외인 -67.1만주 / 기관 +0.3만주 / 개인 +66.3만주 → ⚠️ 개미 물림
  💰 진입 — / 🎯 — / 🛑 ₩11,000
  ⏱️ 단기
...
def build_context(cfg: dict, news_hours: int = 24) -> tuple[str, list[Position], list[dict]]:
    ...
    lines = [f"# 오늘 시각: {timez.now().strftime('%Y-%m-%d %H:%M')} (KST)\n"]
        ...
        if snap:
            ...
            if flow:
                try:
                    ...
                    pass
        # ── 통계 기반 역사적 도달률 주입 (Gemini 가 확률 지어내는 것 방지) ──
        # 목표/손절 범위는 종목별 일변동폭 기반 자동 산출:
        #   목표 = daily_range_pct × 0.5 (장중 평균 움직임의 절반)
        #   손절 = daily_range_pct × 0.4
        # 갭 조건부는 장전 갭 정보 있을 때만 (아직은 무조건부만)
        if snap:
            target_pct = max(0.7, min(5.0, snap.daily_range_pct * 0.5))
            stop_pct = max(0.7, min(5.0, snap.daily_range_pct * 0.4))
            prob = estimate_probability(
                stock["code"], stock["market"],
                target_pct=target_pct, stop_pct=stop_pct,
                gap_pct=None,   # 갭 예측은 Gemini 몫, 여기선 무조건부
                days=90,
            )
            if prob:
                anchor = anchor_probability(prob)
                lines.append(prob.summary_line())
                lines.append(
                    f"  ◦ 앵커 확률(도달률 기반): {anchor}% "
                    "← Gemini 는 이 값 ±3% 내에서만 상승확률 산출"
                )
        lines.append("최근 뉴스:")
...
def _build_stocks_template(stocks: list[dict], mode: str) -> str:
    ...
    for s in stocks:
        ...
        if mode == "day_trade":
            blocks.append(
                f"• {name} [신호] | 상승확률 [N]%\n"
                f"  └ 한 줄 근거 (미국장 반응 + 뉴스 요점)\n"
                f"  💹 수급: 외인 [±N만주] / 기관 [±N만주] / 개인 [±N만주] → [패턴 판정]\n"
                f"  📊 전일 종가 ₩[확정값] / 갭 예상 [±N%] / 일변동 [X.X]%\n"
                f"  📈 예상 시가 ≈ ₩[전일종가×(1+갭%), 반올림 규칙 적용]\n"
                f"  💰 체결: 시가 ±0.5% ≈ ₩[체결하한]~₩[체결상한]\n"
                f"  🎯 목표: 시가 +[Y]% ≈ ₩[목표가] (1주당 +₩[수익])\n"
                f"  🛑 손절: 시가 -[Z]% ≈ ₩[손절가] (1주당 -₩[손실])\n"
                f"  ⏱️ 데이 (9:00~11:00)"
            )
        else:
            blocks.append(
...
def generate_briefing(cfg: dict, slot: str, mode: str = "standard") -> str:
    ...
    # ── 이전 예측의 실적 채점 (하루 한 번만 실제로 돎, 실패 무시) ──
    try:
        _scoring.score_unresolved()
    except Exception as e:
        _log(f"채점 실패 (무시): {e}")
    ...
    accuracy_block = ""
    try:
        accuracy_block = "\n\n# 자가교정 피드백\n" + _scoring.format_accuracy_for_prompt(30) + "\n"
    except Exception:
        pass
    ...
    # ── 상승확률 이중 클램프 ──
    # 1) 신호별 허용 범위(52~58 등) 내로 맞춤
    # 2) 종목별 앵커 확률(역사적 도달률)의 ±3% 내로 타이트하게 조임
    raw = _clamp_probabilities(raw, mode, watchlist)

    # ── 예측 파싱 + 저장 (다음날 채점용) ──
    # day_trade 슬롯만 정확한 목표/손절 파싱 가능. medium 은 포맷 달라서 스킵.
    if mode == "day_trade":
        try:
            preds = _scoring.parse_briefing_predictions(raw, slot, watchlist)
            if preds:
                from src.storage import predictions_store
                predictions_store.save_batch(preds)
                _log(f"예측 로그 저장: {len(preds)}건")
        except Exception as e:
            _log(f"예측 저장 실패 (무시): {e}")

    return raw


# 신호별 허용 확률 범위 (현실적 상한·하한). Gemini 가 어기면 사후 교정.
# mode 별 상한은 조금 다름 (day_trade 는 더 좁음, medium 은 약간 여유).
_PROB_RANGES = {
    "day_trade": {
        "🟢": (52, 58), "🟡": (48, 54), "⚪": (45, 52),
        "🟠": (38, 48), "🔴": (28, 42),
...
def _clamp_probabilities(text: str, mode: str, watchlist: list[dict] | None = None) -> str:
    """'상승확률 N%' 중 범위를 벗어난 숫자를 강제로 범위 내로 교정.

    2중 클램프:
      1) 신호별 허용 범위(🟢=52~58 등) 내로
      2) watchlist 가 주어지면 종목별 앵커 확률(도달률 기반)의 ±3% 내로
         → Gemini 가 지어낸 값이 역사적 실측과 크게 어긋나지 않게
    """
    ranges = _PROB_RANGES.get(mode, _PROB_RANGES["medium"])
    lines = text.splitlines()
    pat = re.compile(r"상승확률\s*(\d+)\s*%")

    # 종목별 앵커 확률 미리 계산 (watchlist 있을 때만)
    ...
    if watchlist:
        for s in watchlist:
            try:
                ...
                if est:
                    anchors[s["name"]] = anchor_probability(est)
            except Exception:
                pass
    ...
    for ln in lines:
        ...
        for emoji in ("🟢", "🟡", "⚪", "🟠", "🔴"):
            if emoji in ln:
                signal_emoji = emoji
                break
        if signal_emoji and signal_emoji in ranges:
            lo, hi = ranges[signal_emoji]
            pct = max(lo, min(hi, pct))

        # 2) 종목 앵커 ±3% 클램프 (종목명 매칭되는 앵커 있을 때만)
