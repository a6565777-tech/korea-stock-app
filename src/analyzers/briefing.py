...
[★★ 수급 분석 - 이 브리핑에서 가장 중요한 시그널 ★★]
뉴스는 이미 시장에 반영됐을 수 있지만 외인·기관·개인 수급은 "지금 이 순간 누가 들어오고
누가 나가는지"를 보여주는 가장 직접적인 선행지표. 수급 패턴을 **신호 결정의 첫 번째 기준**
으로 사용하고, 뉴스는 그걸 보강하는 역할.

[수급 패턴별 룰 - 엄격히 적용]
A. 🔥 외인 + 기관 둘 다 순매수 (2일 이상 연속이면 더 강함)
   → 기본 🟢 매수 추천 후보. 뉴스 보통이어도 OK.
   → 상승확률 +15% (연속 3일 이상이면 +20%)

B. ❄️ 외인 + 기관 둘 다 순매도
   → 무조건 🟠 매수 비추천. 연속·수량 크면 🔴 매도 고려.
   → 상승확률 -15% (연속 3일 이상이면 -20%)

C. ⚠️ 외인·기관 매도 + 개인만 매수 (전형적 "개미 물림")
   → 하락 지속 가능성 가장 높음. 절대 🟢 금지. 🟠/🔴로 확정.
   → 상승확률 -20%

D. 🟡 외인 매수 / 기관 매도 (또는 반대) - 엇갈림
   → ⚪ 관망이 기본. 뉴스·갭이 매우 명확할 때만 다른 신호.
   → 상승확률 ±5% 내외

E. 수급 데이터 없음("수급 데이터 없음" 표시)
   → 근거 라인에 "수급 확인 불가"로 기재, 신호 한 단계 보수적으로.

[수급 기반 신호 오버라이드 규칙]
- 뉴스가 호재여도 패턴 B 또는 C면 🟢/🟡 불가. 최대 ⚪.
- 뉴스가 악재여도 패턴 A면 🟠 금지. ⚪ 또는 그보다 위.
- 대형주(삼성전자·SK하이닉스·현대차·LG전자): 수급이 뉴스보다 훨씬 강한 시그널
- 테마주(휴림로봇 등): 수급 변동 커도 반드시 참고. 특히 외인 대량 매도는 강한 경고.
...
[★★ 수급 분석 규칙 - 가장 중요 ★★]
외국인·기관·개인 수급은 단기 주가의 가장 강한 선행지표. 신호 결정 첫 번째 기준.

패턴별 룰:
  A. 🔥 외인+기관 둘 다 순매수 → 🟢 매수 추천 후보. +15% (3일 연속이면 +20%)
  B. ❄️ 외인+기관 둘 다 순매도 → 🟠 매수 비추천 / 심하면 🔴 매도 고려. -15~20%
  C. ⚠️ 외기관 매도 + 개인만 매수 → "개미 물림" 패턴. 절대 🟢 금지. -20%
  D. 🟡 외인·기관 엇갈림 → ⚪ 관망이 기본. ±5%
  E. 수급 데이터 없음 → "수급 확인 불가" 명시 + 보수적 판단

오버라이드: 뉴스 호재여도 패턴 B/C면 🟢 불가 (최대 ⚪).
           뉴스 악재여도 패턴 A면 🟠 금지 (⚪ 이상).
대형주(삼성전자·SK하이닉스·현대차·LG전자): 수급이 뉴스보다 더 강한 단기 신호.
테마주(휴림로봇 등): 외인 대량 매도·매수는 반드시 체크.
